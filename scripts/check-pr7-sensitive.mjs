...
if (diffFile) {
  ...
  const localPaths = [
    "backend/google-apps-script/PR7FormOperations.gs", "package.json", ".github/workflows/pr-validation.yml",
  ...
  sources = localPaths.map(path => ({ path, text: readFileSync(join(root, path), "utf8") }));
}
const patterns = [
  /sk-[A-Za-z0-9_-]{20,}/g,
  /ya29\.[A-Za-z0-9_-]+/g,
